...
import { HappinessRequestBody, postData } from '@/libs/fetch'
...
const HappinessInput: React.FC = () => {
      ...
      if (missingFields.length > 0) {
        setErrors((prev) => {
          return [
            ...
            {
              field: 'image',
              message: `画像から${missingFields.join(', ')}を取得できませんでした。`,
            },
          ]
        })
        return
  ...
  const submitForm = async () => {
    try {
      ...
      if (mode === 'past') {
        if (!exif) throw new Error('Exif data is missing')
        payload.latitude = exif.latitude
        payload.longitude = exif.longitude
        payload.timestamp = exif.timestamp
