...
import { NgsiMetadataItem } from './interface/change-histories.interface';
...
export class ChangeHistoriesService {
  ...
  async processNotification(
    ...
    for (const entity of notification.data) {
      ...
      try {
        ...
        throw new EntitySaveFailedException(entity.id, error);
      }
    }
  }
  ...
  private convert(entity: OrionNotificationEntityDto, servicePath: string) {
    return {
      entityId: entity.id,
  ...
  private toMetadataArray(metadata?: Record<string, NgsiMetadataItem>) {
    if (!metadata) {
      return [];
    }
