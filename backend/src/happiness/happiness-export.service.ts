...
export class HappinessExportService {
  ...
  findAll(): Promise<Happiness[]> {
    return this.happinessRepository.find();
  }
  ...
  async exportCsv(): Promise<Uint8Array> {
    ...
    const csvData = records.map((record) => {
      ...
      return `${record.nickname},${record.age},${record.address},${timestamp},${jsonLocation.coordinates[1]},${jsonLocation.coordinates[0]},${jsonLocationMd[0].value},${record.happiness1},${record.happiness2},${record.happiness3},${record.happiness4},${record.happiness5},${record.happiness6}`;
    });
